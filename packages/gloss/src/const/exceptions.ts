...
5. part of と思います or similar constructions
`,
  "〜って": `
...
2. "what"
  `,
  "〜ところ": `
1. ところだ - a place is in a location which takes a certain amount of time to get to (~ place where it takes; ~ to get to)
2. ところだ - s.o. / s.t. is in the state where he / it is just about to do s.t., is doing s.t., has done s.t., or has been doing s.t. (be just about to do s.t.; be in the midst of doing s.t.; have just done s.t. etc)
3. ところが - a conjunction which is used to present what in fact happened or what is in fact the case when something else was / is expected (however; but)
4. たところで - even if an action or a state were realized (even if)
5. ところで - indicates a change of topic or a transition (by the way, speaking of which)
  `,
  "〜ために": `
1. for (the sake of)
1. because of
