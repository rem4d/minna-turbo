      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-4 p-2 bg-white rounded-[10px] mb-[100px]">
          {vocabList?.map((vocab, i) => (
            <React.Fragment key={`${vocab.basic_form}-${i}`}>
              <div className="flex items-start space-x-2">
                <div className="whitespace-nowrap text-base text-denim">
                  <div
                    dangerouslySetInnerHTML={{ __html: vocab.ruby ?? "" }}
                  ></div>
                </div>
              ...
              <div className="whitespace-nowrap text-[18px]">
                {vocab.basic_form === vocab.ruby ? "" : vocab.basic_form}
              </div>
              <div className="text-sm">{vocab.en}</div>
            </React.Fragment>
          ))}
        </div>
      </div>
